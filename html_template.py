...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 relative z-1">
        ...
        <div class="flex gap-2 mb-8 bg-white/[0.02] border border-white/[0.05] p-1.5 rounded-2xl max-w-xl mx-auto sm:mx-0 overflow-x-auto whitespace-nowrap">
            ...
            <button id="tab-watchlist" onclick="switchSection('watchlist')" class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-300 text-gray-400 hover:text-white hover:bg-white/[0.04] min-w-[125px]">
                <i class="fas fa-star text-amber-400"></i><span>Watchlist</span>
                <span id="nav-watchlist-count" class="hidden px-2 py-0.5 text-[0.65rem] font-bold rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">0</span>
            </button>
        </div>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex rounded-xl bg-white/[0.02] border border-white/[0.05] p-1 text-xs overflow-x-auto max-w-full">
                        ...
                        <button onclick="setMFFilter('watchlist')" id="btn-mf-watchlist" class="px-3 py-1.5 rounded-lg font-semibold transition-all duration-200 text-gray-400 hover:text-white flex items-center gap-1.5"><i class="fas fa-star text-amber-400 text-[0.65rem]"></i>Watchlist</button>
                    </div>
                </div>
            </div>
        ...
        <!-- Watchlist Section -->
        <section id="watchlist-section" class="hidden mb-8 animate-fade-in">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
                <div>
                    <h2 class="text-base font-bold tracking-tight flex items-center gap-2">
                        <i class="fas fa-star text-amber-400"></i> My Personal Watchlist
                    </h2>
                    <p class="text-xs text-gray-500 mt-0.5">Real-time live prices, NAVs & AI trend signals for your starred instruments</p>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    <!-- Watchlist Category Filter Buttons -->
                    <div class="flex rounded-xl bg-white/[0.02] border border-white/[0.05] p-1 text-xs">
                        <button onclick="setWatchlistCategoryFilter('all')" id="btn-wl-all" class="px-3 py-1.5 rounded-lg font-semibold transition-all duration-200 tf-active">All (<span id="wl-count-all">0</span>)</button>
                        <button onclick="setWatchlistCategoryFilter('stocks')" id="btn-wl-stocks" class="px-3 py-1.5 rounded-lg font-semibold transition-all duration-200 text-gray-400 hover:text-white">Stocks (<span id="wl-count-stocks">0</span>)</button>
                        <button onclick="setWatchlistCategoryFilter('mf')" id="btn-wl-mf" class="px-3 py-1.5 rounded-lg font-semibold transition-all duration-200 text-gray-400 hover:text-white">Mutual Funds (<span id="wl-count-mf">0</span>)</button>
                        <button onclick="setWatchlistCategoryFilter('indices')" id="btn-wl-indices" class="px-3 py-1.5 rounded-lg font-semibold transition-all duration-200 text-gray-400 hover:text-white">Indices (<span id="wl-count-indices">0</span>)</button>
                    </div>
                </div>
            </div>

            <!-- Quick Search Bar inside Watchlist -->
            <div class="glass-card p-3 mb-6 flex items-center gap-3">
                <i class="fas fa-magnifying-glass text-gray-500 text-sm ml-2"></i>
                <input type="text" id="wl-quick-search" placeholder="Search & quick filter any Stock (e.g. RELIANCE, TCS), Mutual Fund (e.g. PPFCF) or Index..." oninput="filterWatchlistSearch(this.value)" class="w-full bg-transparent border-none text-xs text-white placeholder-gray-500 focus:outline-none">
                <button onclick="clearWatchlistSearch()" id="wl-clear-search" class="hidden text-gray-500 hover:text-white text-xs px-2"><i class="fas fa-times"></i></button>
            </div>
            
            <div id="watchlist-content" class="animate-fade-in">
                <!-- Dynamically populated by renderWatchlistSection() -->
            </div>
        </section>

        <!-- Historical Values -->
        <section id="historical-section" class="glass-card p-5 mb-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5">
                <div>
        ...
        <!-- Candlestick Chart -->
        <section id="candlestick-section" class="glass-card p-5 mb-8">
...
let currentChartType = 'candle';
        ...
        function switchSection(section) {
            ...
            if (!['indices', 'stocks', 'mf', 'watchlist'].includes(section)) return;
            ...
            const chartSection = document.getElementById('candlestick-section');
            
            // Update dropdown selects with current category items
            updateDropdownOptions(section);
            
            if (section === 'indices') {
                if (histSection) { histSection.style.display = 'block'; histSection.classList.remove('hidden'); }
                if (chartSection) { histSection.style.display = 'block'; chartSection.style.display = 'block'; chartSection.classList.remove('hidden'); }
                const activeSymbol = 'NIFTY_50';
                currentChartSymbol = activeSymbol;
                const chartSelect = document.getElementById('chart-symbol-select');
                const histSelect = document.getElementById('historical-symbol-select');
                if (chartSelect) chartSelect.value = activeSymbol;
                if (histSelect) histSelect.value = activeSymbol;
                loadHistoricalData();
                loadCandleData(activeSymbol, currentTimeframe);
            } else if (section === 'stocks') {
                if (histSection) { histSection.style.display = 'block'; histSection.classList.remove('hidden'); }
                if (chartSection) { chartSection.style.display = 'block'; chartSection.classList.remove('hidden'); }
                loadStocksData();
                ...
                // In Mutual Funds section: hide candlestick chart & day-by-day table
                if (histSection) { histSection.style.display = 'none'; histSection.classList.add('hidden'); }
                if (chartSection) { chartSection.style.display = 'none'; chartSection.classList.add('hidden'); }
                loadMutualFundsData();
        ...
        let currentWatchlistCategory = 'all'; // 'all', 'stocks', 'mf', 'indices'
        let watchlistSearchQuery = '';
        let watchlistCachedData = { stocks: [], mutual_funds: [], indices: [] };
        ...
        async function loadWatchlist() {
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.success && data.data && Array.isArray(data.data.symbols)) {
                        userWatchlist = data.data.symbols.map(s => s.toUpperCase());
                    } else if (data.symbols && Array.isArray(data.symbols)) {
                        userWatchlist = data.symbols.map(s => s.toUpperCase());
                    }
                }
            } catch(e) {
                console.warn('Watchlist fetch warning:', e);
            }
        ...
        function isWatched(symbol) {
            ...
            return userWatchlist.includes(symbol.toUpperCase());
        }

        function updateWatchlistBadges() {
            const count = userWatchlist.length;
        ...
        function syncAllStarIcons() {
            document.querySelectorAll('[id^="star-"]').forEach(btn => {
                const sym = btn.id.replace('star-', '').toUpperCase();
                ...
                if (icon) {
                    if (isWatched(sym)) {
                        ...
                        icon.className = 'far fa-star text-gray-500 hover:text-amber-400';
        ...
        async function toggleWatchlistEvent(e, symbol) {
            ...
            if (!token || token === 'null' || token === 'undefined') {
                ...
                showToast("Please login to manage your personal watchlist", "warning");
                return;
            }
            ...
            const currentTab = document.querySelector('#tab-watchlist.active, #tab-stocks.active, #tab-mf.active');
            if (document.getElementById('tab-watchlist') && document.getElementById('tab-watchlist').classList.contains('active')) {
                renderWatchlistSection();
            } else if (currentStockFilter === 'watchlist') {
                renderStocksGrid();
            } else if (currentMFFilter === 'watchlist') {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (resData.data && Array.isArray(resData.data.symbols)) {
                        userWatchlist = resData.data.symbols.map(s => s.toUpperCase());
                        syncAllStarIcons();
                        updateWatchlistBadges();
                    }
                }
            } catch(err) {
                console.error('Watchlist API request error:', err);
            }
        }

        function setWatchlistCategoryFilter(cat) {
            currentWatchlistCategory = cat;
            const catMap = {
                'all': 'btn-wl-all',
                'stocks': 'btn-wl-stocks',
                'mf': 'btn-wl-mf',
                'indices': 'btn-wl-indices'
            };
            for (const [key, btnId] of Object.entries(catMap)) {
            ...
            displayWatchlistData(watchlistCachedData.stocks, watchlistCachedData.mutual_funds, watchlistCachedData.indices);
        }

        function filterWatchlistSearch(val) {
            watchlistSearchQuery = (val || '').toLowerCase().trim();
            const clearBtn = document.getElementById('wl-clear-search');
            if (clearBtn) {
                if (watchlistSearchQuery) clearBtn.classList.remove('hidden');
        ...
        function clearWatchlistSearch() {
            const input = document.getElementById('wl-quick-search');
            if (input) input.value = '';
            filterWatchlistSearch('');
        }
        ...
        async function renderWatchlistSection() {
            ...
            if (!userWatchlist || userWatchlist.length === 0) {
                container.innerHTML = `
                    <div class="col-span-full py-16 px-4 text-center glass-card border border-white/[0.08] rounded-3xl animate-fade-in">
                        <div class="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center bg-amber-500/10 border border-amber-500/20">
                            <i class="fas fa-star text-3xl text-amber-400"></i>
                        </div>
                        <h3 class="text-lg font-bold text-gray-100 mb-2">Your Personal Watchlist is Empty</h3>
                        <p class="text-xs text-gray-400 max-w-md mx-auto mb-6 leading-relaxed">
                            Click the star <i class="far fa-star text-amber-400"></i> icon on any Stock, Mutual Fund, or Index across the trading platform to monitor real-time prices, NAVs, and AI predictions here.
                        </p>
                        <div class="flex flex-wrap items-center justify-center gap-3">
                            <button onclick="switchSection('stocks')" class="btn btn-blue text-xs"><i class="fas fa-building mr-1.5"></i> Browse Stocks</button>
                            <button onclick="switchSection('mf')" class="btn btn-purple text-xs"><i class="fas fa-cubes mr-1.5"></i> Explore Mutual Funds</button>
                            <button onclick="switchSection('indices')" class="btn btn-ghost text-xs"><i class="fas fa-chart-line mr-1.5"></i> View Indices</button>
                        </div>
                    </div>
                `;
                updateWatchlistCounts(0, 0, 0, 0);
                return;
            }

            container.innerHTML = `
                <div class="col-span-full py-16 text-center text-gray-400 glass-card">
                    <div class="animate-spin rounded-full h-8 w-8 border-2 border-transparent border-t-amber-400 mx-auto mb-3"></div>
                    <p class="text-xs font-medium">Fetching real-time quotes & AI forecasts for your watchlist...</p>
                </div>
            `;
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (json.success && json.data) {
                        watchlistCachedData = json.data;
                        displayWatchlistData(json.data.stocks || [], json.data.mutual_funds || [], json.data.indices || []);
                        return;
                    }
                }
            } catch(e) {
                console.warn("Failed to load rich watchlist details, using fallback:", e);
            }

            // Fallback if details endpoint unavailable
            displayWatchlistFallback();
        }

        function updateWatchlistCounts(allCount, stocksCount, mfCount, indicesCount) {
            const elAll = document.getElementById('wl-count-all');
            const elStocks = document.getElementById('wl-count-stocks');
            const elMF = document.getElementById('wl-count-mf');
            const elIndices = document.getElementById('wl-count-indices');
            if (elAll) elAll.textContent = allCount;
            if (elStocks) elStocks.textContent = stocksCount;
            if (elMF) elMF.textContent = mfCount;
            if (elIndices) elIndices.textContent = indicesCount;
        }

        function displayWatchlistData(stocks, mutual_funds, indices) {
            const container = document.getElementById('watchlist-content');
            if (!container) return;

            let filteredStocks = stocks.filter(s => isWatched(s.symbol));
            let filteredMF = mutual_funds.filter(f => isWatched(f.symbol));
            let filteredIndices = indices.filter(i => isWatched(i.symbol));

            // Apply search query filter
            if (watchlistSearchQuery) {
                const q = watchlistSearchQuery;
                filteredStocks = filteredStocks.filter(s => (s.symbol && s.symbol.toLowerCase().includes(q)) || (s.name && s.name.toLowerCase().includes(q)) || (s.display_name && s.display_name.toLowerCase().includes(q)));
                filteredMF = filteredMF.filter(f => (f.symbol && f.symbol.toLowerCase().includes(q)) || (f.name && f.name.toLowerCase().includes(q)) || (f.category && f.category.toLowerCase().includes(q)));
                filteredIndices = filteredIndices.filter(i => (i.symbol && i.symbol.toLowerCase().includes(q)) || (i.name && i.name.toLowerCase().includes(q)) || (i.display_name && i.display_name.toLowerCase().includes(q)));
            }

            const totalActive = filteredStocks.length + filteredMF.length + filteredIndices.length;
            updateWatchlistCounts(userWatchlist.length, stocks.filter(s => isWatched(s.symbol)).length, mutual_funds.filter(f => isWatched(f.symbol)).length, indices.filter(i => isWatched(i.symbol)).length);

            if (totalActive === 0) {
                container.innerHTML = `
                    <div class="col-span-full py-12 px-4 text-center glass-card border border-white/[0.08] rounded-2xl">
                        <i class="fas fa-filter text-2xl text-gray-500 mb-2"></i>
                        <p class="text-sm font-medium text-gray-300">No matching items in your watchlist.</p>
                        <p class="text-xs text-gray-500 mt-1">Try selecting another category filter or clearing your search term.</p>
                    </div>
                `;
                return;
            }
            ...
            // 1. Watched Stocks
            if ((currentWatchlistCategory === 'all' || currentWatchlistCategory === 'stocks') && filteredStocks.length > 0) {
                html += `
                    <div class="mb-8">
                        <div class="flex items-center gap-2 mb-4">
                            <span class="w-2 h-2 rounded-full bg-emerald-400"></span>
                            <h3 class="text-sm font-bold text-gray-200 uppercase tracking-wider">Watched Stocks (${filteredStocks.length})</h3>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            ${filteredStocks.map(stock => createStockCardHTML(stock)).join('')}
                        </div>
                    </div>
                `;
            }

            // 2. Watched Mutual Funds
            if ((currentWatchlistCategory === 'all' || currentWatchlistCategory === 'mf') && filteredMF.length > 0) {
                html += `
                    <div class="mb-8">
                        <div class="flex items-center gap-2 mb-4">
                            <span class="w-2 h-2 rounded-full bg-teal-400"></span>
                            <h3 class="text-sm font-bold text-gray-200 uppercase tracking-wider">Watched Mutual Funds (${filteredMF.length})</h3>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            ${filteredMF.map(fund => createMFCardHTML(fund)).join('')}
                        </div>
                    </div>
                `;
            }
            ...
            // 3. Watched Indices
            if ((currentWatchlistCategory === 'all' || currentWatchlistCategory === 'indices') && filteredIndices.length > 0) {
                html += `
                    <div class="mb-8">
                        <div class="flex items-center gap-2 mb-4">
                            <span class="w-2 h-2 rounded-full bg-blue-400"></span>
                            <h3 class="text-sm font-bold text-gray-200 uppercase tracking-wider">Watched Market Indices (${filteredIndices.length})</h3>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            ${filteredIndices.map(symData => {
                                const sym = symData.symbol;
                                const cfg = (typeof INDEX_CONFIGS !== 'undefined' && INDEX_CONFIGS[sym]) ? INDEX_CONFIGS[sym] : { name: symData.name || sym, exchange: symData.exchange || 'NSE', category: symData.category || 'Benchmark', cardClass: 'card-blue', tagClass: 'tag-blue' };
                                const price = Number(symData.price) || 0;
                                const ch = Number(symData.change) || 0;
                                const pct = Number(symData.change_percent) || 0;
                                const priceStr = (cfg.exchange === 'US' ? '$' : '₹') + price.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                                const changeClass = ch >= 0 ? 'profit' : 'loss';
                                const changeArrow = ch >= 0 ? '▲' : '▼';
                                const changeStr = `${changeArrow} ${Math.abs(ch).toFixed(2)} (${Math.abs(pct).toFixed(2)}%)`;
                                return `
                                    <div class="glass-card ${cfg.cardClass} p-4 cursor-pointer transition-all duration-300 hover:shadow-emerald-500/20" onclick="selectIndexSymbol('${sym}')">
                                        <div class="flex justify-between items-start mb-3">
                                            <div>
                                                <h2 class="text-[0.95rem] font-bold tracking-tight">${cfg.name}</h2>
                                                <p class="text-[0.65rem] text-gray-500">${cfg.exchange} · ${cfg.category}</p>
                                            </div>
                                            <div class="flex items-center gap-1.5">
                                                <span class="tag ${cfg.tagClass}">${cfg.exchange}</span>
                                                <button onclick="event.stopPropagation(); toggleWatchlistEvent(event, '${sym}')" class="text-gray-500 hover:text-amber-400 focus:outline-none transition-transform active:scale-125 text-xs p-1" id="star-${sym}" title="Remove from Watchlist">
                                        ...
                                        <div class="flex justify-between items-end mb-2">
                                            <div>
                                                <div class="text-[1.3rem] font-extrabold tracking-tight price-value">${priceStr}</div>
                                                <div class="text-xs font-semibold mt-0.5 ${changeClass}">${changeStr}</div>
                                            </div>
                                            <div class="pill-hold px-2.5 py-0.5 rounded-full text-[0.65rem] font-bold">${symData.prediction || 'HOLD'}</div>
                                        </div>
                                    </div>
                                `;
                            }).join('')}
        ...
        function displayWatchlistFallback() {
            const container = document.getElementById('watchlist-content');
            if (!container) return;
            const watchedStocks = (stocksData || []).filter(s => isWatched(s.symbol));
            const watchedFunds = (mfData || []).filter(f => isWatched(f.symbol));
            displayWatchlistData(watchedStocks, watchedFunds, []);
        }
        ...
        function renderMutualFundsGrid() {
            ...
            let fundsToRender = mfData;
            if (currentMFFilter === 'watchlist') {
                fundsToRender = mfData.filter(f => isWatched(f.symbol));
                if (!fundsToRender || fundsToRender.length === 0) {
                    grid.innerHTML = `
                        <div class="col-span-full py-12 text-center text-gray-500 glass-card">
                            <i class="far fa-star text-2xl mb-2 text-amber-400/50"></i>
                            <p class="text-sm font-medium text-gray-300">Your Mutual Funds watchlist is empty.</p>
                            <p class="text-xs text-gray-500 mt-1">Click the star <i class="far fa-star"></i> icon on any mutual fund card to track it!</p>
                        </div>
                    `;
                    return;
                }
            ...
            grid.innerHTML = fundsToRender.map(fund => createMFCardHTML(fund)).join('');
            syncAllStarIcons();
        }
        ...
        function createMFCardHTML(fund) {
            ...
            const isFundWatched = isWatched(fund.symbol);
                        ...
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <div class="flex flex-col items-end gap-1">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <button onclick="toggleWatchlistEvent(event, '${fund.symbol}')" class="text-gray-500 hover:text-amber-400 focus:outline-none transition-transform active:scale-125 text-xs z-10 p-0.5" id="star-${fund.symbol}" title="${isFundWatched ? 'Remove from Watchlist' : 'Add to Watchlist'}">
                                        <i class="${isFundWatched ? 'fas fa-star text-amber-400' : 'far fa-star text-gray-500 hover:text-amber-400'}"></i>
                                    </button>
                                </div>
                ...
                html += `
                    <div id="${cardId}" class="glass-card ${cfg.cardClass} p-4 sm:p-5 cursor-pointer transition-all duration-300 hover:shadow-emerald-500/20" onclick="selectIndexSymbol('${sym}')">
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span class="tag ${cfg.tagClass}">${cfg.exchange}</span>
                                <button onclick="event.stopPropagation(); toggleWatchlistEvent(event, '${sym}')" class="text-gray-500 hover:text-amber-400 focus:outline-none transition-transform active:scale-125 text-xs p-1" id="star-${sym}" title="${isWatched(sym) ? 'Remove from Watchlist' : 'Add to Watchlist'}">
                                    <i class="${isWatched(sym) ? 'fas fa-star text-amber-400' : 'far fa-star text-gray-500 hover:text-amber-400'}"></i>
                                </button>
                            </div>
                        </div>
                        <div class="flex justify-between items-end mb-4">
        ...
        function renderCandlestick(cd) {
            /* ── Groww-style ultra-clean candlestick & graph chart ── */
            if (!cd || !cd.close || !cd.close.length) {
                console.warn("No candle data available to render.");
                return;
            }
            
            const N = cd.close.length;
            const datesX = (cd.dates && cd.dates.length === N) ? cd.dates : 
                           (cd.hover_dates && cd.hover_dates.length === N) ? cd.hover_dates :
                           Array.from({length: N}, (_, i) => i + 1);

            const hoverLabels = (cd.hover_dates && cd.hover_dates.length === N) ? cd.hover_dates : datesX;
            ...
            for (let i = 0; i < N; i += step) {
                ...
                ticktext.push(cd.dates ? cd.dates[i] : datesX[i]);
            }
            if (N > 0 && tickvals[tickvals.length - 1] !== datesX[N - 1]) {
                tickvals.push(datesX[N - 1]);
                ticktext.push(cd.dates ? cd.dates[N - 1] : datesX[N - 1]);
            }

            // Price bounds
            const validLows = (cd.low || []).filter(v => typeof v === 'number' && !isNaN(v) && v > 0);
            const validHighs = (cd.high || []).filter(v => typeof v === 'number' && !isNaN(v) && v > 0);
            const minLow = validLows.length ? Math.min(...validLows) : Math.min(...cd.close);
            const maxHigh = validHighs.length ? Math.max(...validHighs) : Math.max(...cd.close);
            const priceSpan = maxHigh - minLow || (minLow * 0.05) || 10;
            const yPadding = priceSpan * 0.10;
            const yMin = Math.max(0, minLow - yPadding);
            ...
            if (currentChartType === 'graph') {
                mainTrace = {
                    ...
                    text: hoverLabels,
                    hovertemplate: '%{text}<br><b>Price: ₹%{y:,.2f}</b><extra></extra>'
                };
            } else {
                mainTrace = {
                    x: datesX,
                    open:  cd.open && cd.open.length === N ? cd.open : cd.close,
                    high:  cd.high && cd.high.length === N ? cd.high : cd.close,
                    low:   cd.low && cd.low.length === N ? cd.low : cd.close,
                    close: cd.close,
                    type: 'candlestick',
                    name: 'Price',
                    text: hoverLabels,
            ...
            const volColors = (cd.close || []).map((c, i) => {
                const o = (cd.open && cd.open[i]) ? cd.open[i] : c;
                return c >= o ? 'rgba(16, 185, 129, 0.25)' : 'rgba(239, 68, 68, 0.25)';
            });

            const volumeTrace = {
                x: datesX,
                y: cd.volume || Array(N).fill(0),
                type: 'bar',
                name: 'Volume',
            ...
            const layout = {
                ...
                margin: { t: 15, r: 55, b: 25, l: 10 },
                showlegend: false,
                dragmode: 'pan',
                hovermode: 'x unified',
                hoverlabel: {
                    bgcolor: 'rgba(10, 17, 15, 0.95)',
            ...
            Plotly.newPlot(chartEl, [mainTrace, volumeTrace], layout, {
                displayModeBar: true,
                displaylogo: false,
                ...
                // Ensure chart resizes and fills the parent element correctly
        ...
        function showSymbolSelection() {
            ...
            for (const [key, cfg] of Object.entries(INDEX_CONFIGS)) {
                indexCardsHTML += `
                    <button onclick="selectSymbol('${key}')" class="ai-stock-pick glass-card p-3.5 text-left cursor-pointer hover:scale-[1.02] hover:border-teal-500/40 transition-all duration-200" style="border:1px solid rgba(255,255,255,0.08)">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-teal-500/10 border border-teal-500/20">
                                <i class="fas fa-chart-line text-xs text-teal-400"></i>
                            </div>
                            <div class="truncate">
                                <div class="text-sm font-bold truncate text-white">${cfg.name}</div>
                                <div class="text-[0.65rem] text-gray-500">${cfg.category}</div>
                            </div>
                        </div>
                        <div class="flex items-center justify-between mt-1">
                            <span class="text-[0.65rem] uppercase tracking-wider px-2 py-0.5 rounded font-mono font-semibold bg-white/[0.04] text-gray-300 border border-white/[0.06]">${cfg.exchange}</span>
                            <span class="text-[0.65rem] text-teal-400 font-medium">Analyze AI →</span>
                        </div>
                    </button>`;
            }
            ...
            content.innerHTML = `
                <div class="text-center mb-6">
                    <h3 class="text-base font-bold mb-1 flex items-center justify-center gap-2 text-white">
                        <i class="fas fa-brain text-teal-400"></i> Select Market Index for AI Analysis
                    </h3>
                    <p class="text-xs text-gray-400">Deep learning predictions, BUY/SELL/HOLD recommendations & Options Chain</p>
                </div>

                <!-- Indices Grid -->
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6" style="max-height:55vh;overflow-y:auto;padding-right:4px;">
                    ${indexCardsHTML}
                </div>

                <div class="text-center">
                    <button onclick="closeAIModal()" class="btn btn-ghost text-xs px-6 py-2">Close</button>
                </div>
            `;
            modal.classList.add('active');
        }
